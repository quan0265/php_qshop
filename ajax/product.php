...
if(isset($_POST['product_more'])){
	...
	$sql= "SELECT COUNT(products.product_id) AS count_product FROM products INNER JOIN brands ON products.brand_id=brands.brand_id INNER JOIN categories ON brands.cate_id=categories.cate_id WHERE categories.cate_id='$cate_id'";

	if(isset($_POST['brand_id']) && $_POST['brand_id']!='0'){
		$brand_id= $_POST['brand_id'];
		$sql= "SELECT COUNT(products.product_id) AS count_product FROM products INNER JOIN brands ON products.brand_id=brands.brand_id INNER JOIN categories ON brands.cate_id=categories.cate_id WHERE categories.cate_id='$cate_id' AND brands.brand_id='$brand_id'";
